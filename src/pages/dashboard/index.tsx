...
const Dashboard = (): JSX.Element => {
  const { errors, isDirty, isValid, watch, onSubmit, register, handleSubmit } =
    useDashboard();
    ...
    <>
            ...
            <div className="text-black flex container w-full h-full">
              ...
              >
                ...
                <div className="flex justify-between text-black">
                  ...
                  <p className="text-right">{watch("order")}</p>
                </div>
                <div className="flex justify-between text-black ">
                  <h2 className="font-bold">Nombre: </h2>{" "}
                  <p className="text-right">{watch("name")}</p>
                </div>
                <div className="flex justify-between text-black ">
                  <h2 className="font-bold">Celular: </h2>{" "}
                  <p className="text-right">{watch("phone")}</p>
                </div>
                <div className="flex justify-between text-black ">
                  <h2 className="font-bold">Descripción: </h2>{" "}
                  <p className="text-right">{watch("description")}</p>
                </div>
                <div className="flex justify-between text-black ">
                  <h2 className="font-bold">Valor: </h2>{" "}
                  <p className="text-right">
                    $ {watch("value") ? numberWithCommas(watch("value")) : 0}
                  </p>
                </div>
                <div className="flex justify-between text-black ">
                  ...
                  <p className="text-right">
                    ${" "}
                    {watch("payment")
                      ? numberWithCommas(watch("payment"))
                      : numberWithCommas(0)}
                  </p>
                </div>
                <div className="flex justify-between text-black ">
                  ...
                  <p className="text-right">
                    ${" "}
                    {watch("payment")
                      ? numberWithCommas(watch("value") - watch("payment"))
                      : 0}
                  </p>
                </div>
                <div className="mt-8">
              ...
              <div className="Card w-[500px] m-auto p-20 shadow-2xl text-center">
                <h2 className="text-3xl font-body p-8 text-black">
                  INGRESA LOS DATOS DEL RECIBO
                </h2>
                <form
      ...
      {isPrint && (
        <div className={`  ${styles.print_area}`} id="print_area">
          <div
            className={`min-w-[160px] w-auto h-14 flex items-center cursor-pointer relative`}
          >
            <Image
              src={sneakerCleanupGrayLogo}
              alt="logo sneaker cleanup"
              layout="fill"
              className="min-w-[180px]"
            />
          ...
          <div className="flex justify-between text-black mb-2">
            ...
            <p className="text-right text-sm">{watch("order")}</p>
          </div>
          <div className="flex justify-between text-black mb-2 ">
            <h2 className="font-bold text-sm">Nombre: </h2>{" "}
            <p className="text-right text-sm">{watch("name")}</p>
          </div>
          <div className="flex justify-between text-black mb-2 ">
            <h2 className="font-bold text-sm">Celular: </h2>{" "}
            <p className="text-right text-sm">{watch("phone")}</p>
          </div>
          <div className="flex justify-between text-black mb-2 ">
          ...
          <div className="flex items-center justify-center flex-col text-center mt-8">
            ...
            <h2 className="font-bold text-xs text-black mt-2">
              Autorizas el tratamiento de datos, aceptas la Política de
              Privacidad y los Términos y condiciones de sneakers cleanup.
            </h2>
